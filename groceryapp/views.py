...
def add_product(request):
    ...
    if request.method == "POST":
        ...
        Product.objects.create(name=name, price=price, discount=discount, category=catobj, description=desc, image=image,quantity=quantity)
...
def edit_product(request, pk):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            pass
        catobj = Category.objects.get(id=cat)
        Product.objects.filter(id=pk).update(name=name, price=price, discount=discount, category=catobj, description=desc,quantity=quantity)
...
def myOrder(request):
    if not request.user.is_authenticated:
        return redirect("userlogin")
    order = Booking.objects.filter(user=request.user).order_by('-created')
...
def Invoice(request):
    ...
    try:
        ...
        product = []
    lengthpro = len(product)
    return render(request, 'paymentdetails.html', locals())
...
def staff_order(request):
    ...
    if int(action) == 0:
        order = Booking.objects.filter()
        order_status = 'All'
    return render(request, 'delivery_order.html', locals()) 


def staff_order_track(request, pid):
    if not request.user.is_staff:
        return redirect("admin_login")
    order = Booking.objects.get(id=pid)
    orderstatus = ORDERSTATUS
    ...
    if status:
        ...
        order.save()
        return redirect('admin_order_track', pid)
    return render(request, 'staff-order-track.html', locals())



def InvoiceView(request, pid):
    order = Booking.objects.filter(id=pid)
    orderstatus = ORDERSTATUS
    status = int(request.GET.get('status',0))
    if status:
        order.status = status
